...
def compute_j_batch(theta_2, theta_3):
	y_hat = np.zeros(train_y[0 : sz_b].shape)
	_, y_hat = forward_propogation(train[0 : sz_b], theta_2, theta_3)
	j = -1 / sz_b * np.sum(train_y[0 : sz_b] * np.log(y_hat) + (1 - train_y[0 : sz_b]) * (np.log(1 - y_hat)))
	j += _lambda / (2 * sz_b) * (np.sum(theta_2[:, 1 :] ** 2) + np.sum(theta_3[:, 1 :] ** 2)) # Regularization term
	return j
...
def gradient_check(delta_2, delta_3):
	...
	for i in range(theta_3.shape[0]):
		...
		for j in range(theta_3.shape[1]):
			...
			y2 = compute_j_batch(theta_2, theta_3 + diff_matrix)
			y1 = compute_j_batch(theta_2, theta_3 - diff_matrix)
			derivative = (y2 - y1) / (2 * epsilon_delta)
...
for t in range(iterations):
	...
	for b in range(m_b):
		...
		a_2, a_3 = forward_propogation(batch_x, theta_2, theta_3)

		delta_3 = a_3 - batch_y # [3000 * 10]
		...
		triangle_2 += np.transpose(delta_2).dot(batch_x)

		D_2 = 1 / sz_b * triangle_2
		D_3 = 1 / sz_b * triangle_3 
		D_2[:, 1 :] += _lambda / sz_b * theta_2[:, 1 :]
		D_3[:, 1 :] += _lambda / sz_b * theta_3[:, 1 :]
